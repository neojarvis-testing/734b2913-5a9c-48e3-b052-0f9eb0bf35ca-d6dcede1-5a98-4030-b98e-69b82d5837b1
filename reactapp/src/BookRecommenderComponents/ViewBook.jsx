...
const ViewBook = () => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setBooks((prevBooks) => prevBooks.filter((book) => book.bookId !== selectedBookId)); // Remove deleted book from state
    } catch (err) {
          ...
          <tbody>
            ...
            {Books.map((book) => (
              <tr key={book.bookId}>
                <td>
                  <img
                    src={book.coverImage ? `data:image/jpeg;base64,${book.coverImage}` : 'https://via.placeholder.com/100'}
